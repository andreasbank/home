...
if(isset($_GET['testMysql'])) {
	...
	// get_bookings()
		...
		if($file != null) {
			// global settings
			$write_buffer = "<?php\r\n";
			$write_buffer = sprintf("%s/**\r\n * This file is automatically generated by the install process.\r\n * Please do not alter it manually!\r\n", $write_buffer);
			$write_buffer = sprintf("%s * DATE GENERATED: %s\r\n */\r\n", $write_buffer, date("Y-m-d H:m:s"));
			$write_buffer = sprintf("%sdefine('MYSQL_HOST', '%s');\r\n", $write_buffer, $_POST['mysql_host']);
			$write_buffer = sprintf("%sdefine('MYSQL_USERNAME', '%s');\r\n", $write_buffer, $_POST['mysql_username']);
			$write_buffer = sprintf("%sdefine('MYSQL_PASSWORD', '%s');\r\n", $write_buffer, $_POST['mysql_password']);
			$write_buffer = sprintf("%sdefine('MYSQL_PORT', '%s');\r\n", $write_buffer, $_POST['mysql_port']);
			$write_buffer = sprintf("%sdefine('DATABASE_NAME', '%s');\r\n", $write_buffer, $_POST['database_name']);
			$write_buffer = sprintf("%sdefine('USERS_TABLE', '%s');\r\n", $write_buffer, $_POST['users_table']);
			$write_buffer = sprintf("%sdefine('SESSIONS_TABLE', '%s');\r\n", $write_buffer, $_POST['sessions_table']);
			$write_buffer = sprintf("%sdefine('BOOKINGS_TABLE', '%s');\r\n", $write_buffer, $_POST['bookings_table']);
			$write_buffer = sprintf("%sdefine('PORTALS_TABLE', '%s');\r\n", $write_buffer, $_POST['portals_table']);
			$write_buffer = sprintf("%sdefine('COOKIE_NAME', '%s');\r\n?>\r\n", $write_buffer, $_POST['cookie_name']);
			fwrite($file, $write_buffer);
			fclose($file);
